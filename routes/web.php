...
Route::get('/operational/supplies/list-mobile', [OperationalSupplyController::class, 'listSupplies'])->middleware('permission:supplies');
Route::put('/operational/supplies/{id}/update-stock-mobile', [OperationalSupplyController::class, 'updateSupplyStockMobile'])->middleware('permission:supplies');
Route::get('/api/mobile/expense/accounts-and-categories', [FinancialMutationController::class, 'getExpenseMetaMobile'])->middleware('permission:expenses');
Route::post('/api/mobile/expense/store', [FinancialMutationController::class, 'storeExpenseMobile'])->middleware('permission:expenses');

Route::middleware(['auth', 'verified'])->group(function () {
    Route::get('/dashboard', [DashboardController::class, 'index'])->name('dashboard');

    // Keuangan & Analisa Admin Only woy!
    ...
    // 2. Modul Stasiun Packing & Proof Uploads woy!
    Route::middleware(['permission:scanner'])->group(function () {
    ...
    // 3. Modul Bahan Operasional woy!
    Route::middleware(['permission:supplies'])->group(function () {
    ...
    // Menu Master Data & Backups Admin Only woy!
    Route::middleware(['role:admin'])->group(function () {
...
Route::middleware(['auth', 'verified', 'role:admin'])->prefix('finance/ads-affiliate')->name('ads-affiliate.')->group(function () {
